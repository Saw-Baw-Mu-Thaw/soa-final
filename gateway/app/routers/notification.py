...
from ..models.InputModels import NotificationSeenInput
...
async def mark_material_seen(notificationIds: List[int]):
    url = NOTI_URL + 'notifications/material'
...
async def mark_homework_seen(notificationIds: List[int]):
    url = NOTI_URL + 'notifications/homework'
    data = {'notificationIds' : notificationIds}
    response = requests.put(url=url, data=json.dumps(data),
                           headers={'Content-Type': 'application/json'})
