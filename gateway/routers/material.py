...
@router.get('/materials', dependencies=[Depends(is_teacher_or_student)])
...
@router.get('/materials/{material_id}', dependencies=[Depends(is_teacher_or_student)])
async def get_specific_material(material_id : Annotated[int, Path(gt=0)]):
    url = MATERIAL_URL + '/materials/' + str(material_id)
    response = requests.get(url=url)
    return response.json()

@router.post('/materials', dependencies=[Depends(is_teacher)])
...
@router.put('/materials/{material_id}', dependencies=[Depends(is_teacher)])
...
@router.delete('/materials/{material_id}', dependencies=[Depends(is_teacher)])
